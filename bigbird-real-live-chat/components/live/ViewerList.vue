<template>
  <div class="side-content">
    <h4 class="side-content-title">
      시청자 목록
      <span>{{ "(" + viewerList.length + ")" }}</span>
    </h4>
    <div class="side-content-box">
      <div class="viewer-list-box">
        <div class="viewer-search-box">
          <span></span>
          <form autocomplete="off">
            <input
              name="search"
              autocomplete="off"
              type="text"
              placeholder="시청자 검색"
              ref="search"
              @input="search($event)"
            />
          </form>
        </div>
        <ul class="viewer-list-scroll" v-if="searchData.length === 0">
          <li v-for="(list, idx) in viewerList" :key="idx">
            <a href="#" @click.prevent="controlBox(list)">{{
              `${list.nickName}(${list.id})`
            }}</a>
          </li>
        </ul>
        <ul class="viewer-list-scroll" v-else>
          <li v-for="(list, idx) in searchList" :key="idx">
            <a href="#" @click.prevent="controlBox(list)">{{
              `${list.nickName}(${list.id})`
            }}</a>
          </li>
        </ul>
      </div>
    </div>
    <con-trol-box
      v-if="false"
      @close="isControlBox = false"
      :controlData="controlData"
    />
  </div>
</template>

<script>
import ConTrolBox from "@/components/live/ConTrolBox";
export default {
  name: "viewerList",
  data() {
    return {
      viewerList: [],
      payload: null,
      config: null,
      chatty: [],
      loggedIn: [],
      timer: null,
      searchList: [],
      searchData: "",
      controlData: null,
      isControlBox: false,
    };
  },
  components: {
    ConTrolBox,
  },
  mounted() {
    this.payload = this.getPayload();
    this.config = this.getConfig();
    // console.log(this.payload);

    this.fb_chat_init(this.config, this.payload);
    this.fb_chat_login();
    this.fb_chat_load_userlist(this.getUserList);
  },

  methods: {
    getPayload() {
      return this.jwt_decode(this.$route.query.token);
    },
    getUserList(user) {
      if (!this.viewerList.find((v) => v.id == user.id)) {
        this.viewerList.push(user);
      }
      this.chatty = this.viewerList.filter((v) => v.chatty === true);
      this.loggedIn = this.viewerList.filter((v) => v.logged_in === true);
      // console.log(this.viewerList);
      window.parent.postMessage(
        {
          status: "userCount",
          chatty: this.chatty.length,
          loggedIn: this.loggedIn.length,
        },
        "*"
      );
    },
    search(e) {
      if (this.timer) {
        clearTimeout(this.timer);
      }

      this.timer = setTimeout(() => {
        this.searchData = e.target.value;
        this.searchList = this.viewerList.filter(
          (v) =>
            v.nickName.toLowerCase().includes(this.searchData.toLowerCase()) ||
            v.id.toLowerCase().includes(this.searchData.toLowerCase())
        );
      }, 200);
    },
    controlBox(data) {
      if (this.controlData) {
        this.controlData = null;
      }
      this.isControlBox = true;
      this.controlData = data;
    },
  },
};
</script>

<style lang="scss" scoped>
input:focus {
  outline: none;
}
.side-content-title {
  background: #fff;
  padding: 40px 20px 20px;
  box-sizing: border-box;
  font-size: 16px;
  color: #000;
  border-bottom: 1px solid #eeeeee;
}
.viewer-list-box {
  > .viewer-search-box {
    height: 40px;
    width: 100%;
    background: #fff;
    border-bottom: 1px solid #eeeeee;
    box-sizing: border-box;
    padding: 0 20px;
    display: flex;
    justify-content: left;
    align-items: center;
    > form {
      width: calc(100% - 24px);
      height: 100%;
      input {
        display: inline-block;
        width: 100%;
        height: 100%;
        color: #000;
        border: none;
        box-sizing: border-box;
      }
      input::placeholder {
        color: #bbbbbb;
      }
    }

    > span {
      display: inline-block;
      width: 17px;
      height: 17px;
      background: url(../../assets/img/dodbo@2x.png) repeat center;
      background-size: cover;
      margin-right: 7px;
    }
  }
  > .viewer-list-scroll {
    overflow-x: hidden;
    overflow-y: auto;
    height: 705px;
    > li {
      padding: 6px 20px;
      box-sizing: border-box;
      font-size: 12px;
      color: #000;
    }
    > li:nth-child(2n-1) {
      background: #f9f9f9;
    }
    > li:nth-child(2n) {
      background: #fff;
    }
  }
}
</style>